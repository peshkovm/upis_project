...
/**
 * This class is a.txt parser of Connection. It outputs the contents of item to the files, also this
 * class saves the value of most recent post in Connection and outputs only newly added posts, when
...
class StaxStreamProcessor {
          ...
          if (str != null) {
            ...
            if (lastpubdate == null
                || lastpubdate.isBefore(
                    pubdate)) { // if it is a.txt new item (the date of the item is after the
              // lastpubdate)
              if (time == null
                  || pubdate.isAfter(
   ...
   */
  private String getElement(final XMLStreamReader reader, final String element)
      throws XMLStreamException {
    final StringBuilder str = new StringBuilder();
    while (reader.hasNext()) {
      if (reader.getEventType() == XMLStreamReader.START_ELEMENT
          && reader.getLocalName().equals(element)) break;
      reader.next();
    }
    ...
    return str.length() == 0
        ? null
        : str.deleteCharAt(str.length() - 1).toString(); // TODO Optional
  }
  ...
  private ZonedDateTime getDate(final String str) {
    final Pattern pattern =
        Pattern.compile(
            "pubDate (.*)"); // gets the content of the pubdate element in RFC-822 format
    final Matcher matcher = pattern.matcher(str);
    matcher.find();
    return ZonedDateTime.parse(
        matcher.group(1), DateTimeFormatter.RFC_1123_DATE_TIME); // converts from the RFC-822
  }
}
